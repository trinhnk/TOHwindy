	...
	function i(i, a, r, o, l, c, d) {
		...
		try {
			...
			if (v > 0 && (p.msg = "(" + v + "x) " + p.msg, p.repeated = v), o && p.script && (p.scriptLine = s(p.script, 10) + "-" + o), W.latestBcasts && W.latestBcasts.length) {
				p.latestBcast = "";
				for (var y = 0; y < W.latestBcasts.length; y++) p.latestBcast += W.latestBcasts[y].txt + " (" + (f - W.latestBcasts[y].ts) + "ms. ago)\n"
			}
			if (m) {
				p.uid = m.user && m.user.username;
				var w = W.store && W.store.get("country") || "xx";
				e || (e = w + "-" + (p.uid ? p.uid : m.sessionCounter)), p.sessionName = e, p.sessionCounter = m.sessionCounter, p.lang = W.store && W.store.get("usedLang"), p.retina = m.isRetina, p.size = window.screen.width + "x" + window.screen.height, p.glParticles = m.glParticlesOn, p.platform = m.platform
			}
			...
			var b = new XMLHttpRequest,
				T = JSON.stringify(p);
			b.open("post", "https://node.windy.com/sedlina/err", !0), b.setRequestHeader("Content-type", "application/json; charset=utf-8"), b.onreadystatechange = function () {
...
function (e) {
	...
	e.require = function (e) {
		if ("string" == typeof e) {
			var i = t[e];
			if (!i) throw new Error('Tinyrequire error: Module "' + e + '" does not exist!');
...
L.CanvasLayer = L.Layer.extend({
	...
	W.define("http", ["lruCache", "rootScope", "utils", "broadcast"], function (e, t, n, i) {
		...
		var c = function (e) {
			...
			u = function (e, o, l) {
				...
				o = encodeURI(o), g.open(e, o, !0), l.binary && (g.responseType = "arraybuffer"), f && (g.withCredentials = !0), g.setRequestHeader("Accept", d), h = new Promise(function (e, t) {
					var n, a = {},
						r = {};
		...
		return i.DD2DMS = function (e, t) {
			...
			var s = Date.now(),
				...
				r = function () {
					...
					t && (t.textContent = i.thousands(o) + " views"), t && o < e && window.requestAnimationFrame(r)
				};
			r()
		}, i
	}), /*! */
		...
		return n.extend({
			...
			createGradientArray: function (e, t, n) {
				...
				var i, s, a, r, o = this.steps + 1,
					...
					m = u[0],
					f = u[h++];
				for (r = 0; r < this.steps; r++)(i = (this.min + c * r) * n) > f[0] && h < u.length && (m = f, f = u[h++]), a = (i - m[0]) / (f[0] - m[0]), s = this.getGradientColorYUVA(m[1], f[1], a), t && this.makePremultiplied(s), l[d++] = Math.round(s[0]), l[d++] = Math.round(s[1]), l[d++] = Math.round(s[2]), l[d++] = e ? 255 : Math.round(s[3]);
	...
	W.define("Product", ["utils", "rootScope", "http", "store", "Class", "Calendar", "Window", "rhMessage"], function (e, t, n, i, s, a, r, o) {
		return s.extend({
			...
			_init: function () {
				var e;
				this.productExpires = 0, this.minifest = {}, this.pathGenerator = (e = {}, e[2] = "{server}/{directory}/{path}/257w<z>/<y>/<x>/{filename}-{level}.{fileSuffix}", e[3] = "{server}/im/v3.0/{directory}/{refTime}/{path}/wm_grid_257/<z>/<x>/<y>/{filename}-{level}.{fileSuffix}", e)[this.imVersion]
			},
			refTime: function () {
			...
			loadMinifest: function (e) {
				var i = (new Date).toISOString().replace(/.*T(\d+):(\d+).*/, "$1$2"),
					...
					a = this.directory,
					r = 3 === this.imVersion ? s + "/im/v3.0/" + a + "/info.json?" + i : s + "/" + a + "/minifest2.json?" + i;
				return n.get(r, {
					cache: !e
				})
			},
			open: function () {
				var e = this;
				return t.isMobileOrTablet || this.printLogo(), this.loadingPromise ? this.loadingPromise : Date.now() < this.productExpires ? Promise.resolve(this.calendar) : (this.loadingPromise = new Promise(function (t) {
	...
	W.define("products", ["Product", "StaticProduct", "Calendar", "$", "rootScope", "radar", "satellite"], function (e, t, n, i, s, a, r) {
		var o = {},
			l = "Â© 2016 ECMWF: This service is based on data and products of the European Centre for Medium-range Weather Forecasts",
			c = "Â© Generated using Copernicus Atmosphere Monitoring Service Information [2019]. Neither the European Commission nor ECMWF is responsible for any use of this information.",
	...
	W.define("rootScope", ["supportedLanguages", "detectDevice"], function (e, t) {
		var n = t.platform,
			...
			a = {
				...
				pointForecast: "v2.5",
				iconsDir: "img/icons4",
				overlays: ["radarSat", "radar", "satellite", "wind", "gust", "gustAccu", "rain", "rainAccu", "snowAccu", "snowcover", "ptype", "thunder", "temp", "dewpoint", "rh", "deg0", "clouds", "hclouds", "mclouds", "lclouds", "fog", "cloudtop", "cbase", "visibility", "cape", "waves", "swell1", "swell2", "swell3", "wwaves", "sst", "currents", "airQ", "no2", "pm2p5", "aod550", "gtco3", "cosc", "dustsm", "so2sm", "fires", "pressure", "efiWind", "efiTemp", "efiRain", "capAlerts", "map"],
	...
	W.define("trans", ["utils", "storage", "rootScope", "store", "seoParser", "langEn"], function (e, t, n, i, s, a) {
		...
		var f = function () {
			o.translateDocument(document.body), m(l), i.on("lang", function (e) {
				"auto" !== e && e !== i.get("usedLang") && m(e)
			})
		};
		return "loading" !== document.readyState ? f() : document.addEventListener("DOMContentLoaded", f), o
	}), /*! */
		...
		var a = {},
			...
			u = function (e, i, s, o) {
				if (null === o ? delete a[e] : a[e] = o, i.save && !s.doNotStore && t.isAvbl) {
					...
					t.put("settings_" + e, o), i.sync && (i.update = l, t.put("settings_" + e + "_ts", l), t.put("lastSyncableUpdatedItem", l)), n.user && i.sync && !s.doNotSaveToCloud && r.emit("_cloudSync")
				}
	...
	W.define("satellite", ["StaticProduct", "Evented", "rootScope", "http"], function (e, t, n, i) {
		return e.instance(t, {
			...
			overlays: ["satellite"],
			levels: ["surface"],
			logo: '<a class="size-m uiyellow inlined" href="https://community.windy.com/topic/8820" data-icon-after=">">This is a <b>beta version</b> of our new satellite layer. Let us know how you like it</a>\n\n\t<a href="https://www.eumetsat.int/" target="_blank"><img src="img/providers/eumetsat2.svg" /></a>\n\t<div class="uiyellow size-m clickable inlined" data-do="rqstOpen,animate" data-icon="&#xe047;">Create video</div>',
			open: function () {
				return n.isMobileOrTable || this.once("loadedAll", this.printLogo.bind(this)), this.loadInfoJson(), Promise.resolve()
			},
	...
	W.define("params", ["store", "models", "overlays", "utils", "broadcast", "products", "router", "renderCtrl"], function (e, t, n, i, s, a, r, o) {
		...
		e.defineProperty("overlay", "asyncSet", function (n) {
			...
			return i === e.get("product") ? (m(n), Promise.resolve(n)) : new Promise(function (t) {
	...
	W.define("router", ["store", "utils", "broadcast", "rootScope", "plugins", "seoParser"], function (e, t, n, i, s, a) {
		...
		return p ? (n.once("paramsChanged", function () {
			return n.emit("rqstOpen", p.plugin, p.params || d)
		}), i.startupDetail = p.params || p.plugin, l = a.startupUrl) : l = "", p && p.plugin || !o || !r || (o.noEmit = !0, n.once("redrawFinished", n.emit.bind(n, "rqstOpen", "picker", o))), {
			url: l,
			url404: u,
			sharedCoords: r,
			parseSearch: m,
	...
	W.define("cloudSync", ["dataSpecifications", "storage", "store", "http", "utils", "broadcast", "rootScope", "colors", "metrics", "user"], function (e, t, n, i, s, a, r, o, l, c) {
		...
		a.on("userLoggedIn", function () {
			i.get("/users/settings?storeTs=" + d, {
				...
				var s = i.status,
					r = i.data,
					o = t.get("lastSyncableUpdatedItem") || 0;
				o > d && g(), 304 !== s && r && r.data && r.version > 1 && (! function (i) {
					var s = !1;
					for (var r in e)
						if (f(r) && h(r) in i) {
							var o = m(r),
				...
				}(r.data), r.storeTs > d && t.put("storedSettings", r.storeTs))
			...
			p = function () {
				...
				for (var r in e)
					if (f(r)) {
						...
						o > d && (s[h(r)] = n.get(r), s[m(r)] = o, a = Math.max(a, o))
					}
				return {
					toStore: s,
					updated: a
	...
	W.define("plugins", ["rootScope", "Plugin", "TagPlugin", "TagAutoOpen", "RiotPlugin"], function (e, t, n, i, s) {
		return {
			...
			"nearest-webcams": n.instance({
				...
				dependencies: ["nearest"],
				className: "drop-down-window left boxshadow normal-closing-x",
				hasURL: !1,
				langFile: "webcams",
				unmountOnClose: !0
			}),
	...
	W.define("DraggableDiv", ["Class", "utils"], function (e, t) {
		return e.extend({
			...
			startDrag: function (e) {
				...
				var u = function () {
						clearInterval(d), window.removeEventListener("mousemove", m), window.removeEventListener("mouseup", u), Math.abs(s) > 10 && (r = .6 * s, o = n.scrollEl.scrollLeft + r, a = Date.now(), n.inertiaAnim = window.requestAnimationFrame(h)), e.preventDefault(), e.stopPropagation()
					},
					h = function () {
						var e = Date.now() - a,
							t = -r * Math.exp(-e / 325);
	...
	W.define("ClickHandler", ["Class", "broadcast", "store"], function (e, t, n) {
		return e.extend({
			...
			defaultClickHandlers: function (e, i) {
				switch (i[0]) {
					case "rqstOpen":
					case "toggle":
					case "rqstClose":
						return t.emit.apply(t, i), e.stopPropagation(), !0;
					case "url":
						return i.shift(), window.location.href = i.join(","), e.stopPropagation(), !0;
					case "share":
						return t.emit("rqstOpen", "share"), !0;
					case "login":
						return W.user.login(), !0;
					case "store":
						return n.set(i[1], i[2]), !0
				}
				return !1
			}
		})
	...
	W.define("Swipe", ["Class"], function (e) {
		return e.extend({
			...
			touchStart: function (e) {
				this.isSwipeValid = !0, this.direction = null, this.x = this.xStart = this.xThrottled = e.touches[0].clientX, this.y = this.yStart = this.yThrottled = e.touches[0].clientY
			},
	...
	W.define("Webcams", ["store", "map", "trans", "utils", "http", "format", "ClickHandler", "broadcast", "metrics"], function (e, t, n, i, s, a, r, o, l) {
		return r.extend({
			...
			useHover: !0,
			_init: function () {
				r._init.call(this), this.converter = l.distance.convertValue.bind(l.distance)
			},
			...
			load: function (t) {
				var n = this;
				return this.removeMarker(), this.currentLocation = L.latLng(t.lat, t.lon), new Promise(function (a) {
					s.get("/webcams/v1.0/list?" + i.qs({
						...
						var t = e.data;
						n.daylight = !1, t.cams.length > 0 ? (n.data = t.cams.slice(0, n.maxAmount), n.render()) : (n.data = [], n.el && (n.el.innerHTML = "")), a(n.data)
					})
				})
			},
			...
			loadImages: function () {
				...
				this.forEach(function (e, i) {
					...
					e.querySelector(".webcam-image").style["background-image"] = "url(" + s + "), url( img/ajax-loader.gif )", e.classList.add("loaded")
				})
			},
			click: function (e, t) {
	...
	W.define("BottomSlide", ["Drag", "broadcast"], function (e, t) {
		return e.extend({
			preventDefault: !1,
			passiveListener: !1,
			threshold: 30,
			...
			ondragend: function () {
				Math.abs(this.transformedY - this.startY) > this.threshold ? (this.pluginEl.style.transition = null, this.pluginEl.style.transform = null, this.difference > 0 ? t.emit("rqstClose", this.pluginName) : this.pluginEl.classList.remove("open-half")) : this.updatePosition(this.startY)
			}
		})
	}), /*! */
	W.define("location", ["trans", "store", "$", "utils", "broadcast", "rootScope", "router", "picker", "overlays", "format", "plugins"], function (e, t, n, i, s, a, r, o, l, c, d) {
		var u, h = c.seoString,
			...
			g = Date.now(),
			v = "Windy: " + e.TITLE,
			y = n('meta[name="description"]') || {},
			w = y && y.content,
			b = !1,
			T = n('link[rel="canonical"]'),
			L = null,
			S = null,
			E = null;
		"/" === p && (p = "");
		...
		a.isCrawler ? d.seo.load().then(function () {
			E = W.require("seo"), x = E.showURL
		}) : window.history && window.history.replaceState && (x = i.debounce(function () {
			try {
				var e = P();
		...
		var P = function () {
				...
				return /^\/?(overlays|settings|privacy|tools|widgets|favs|hurricanes|webcams)/.test(p) && (L || S) ? S ? i.joinPath("" + m(t.get("usedLang")) + f(S), p) : L ? i.joinPath(_(), p) : void 0 : p
			},
			_ = function () {
			...
			R = i.debounce(function () {
				...
				var e, n = a.map,
					...
					r = t.get("timestamp");
				"accumulations" === O.product && "next24h" !== O.acTime ? s.unshift(O.acTime) : O.path && Math.abs(g - r) > 65e6 && s.unshift(O.path.replace(/\//g, "-"));
				"wind" !== O.overlay && s.unshift(O.overlay);
				"surface" !== O.level && s.unshift(O.level);
				!/^ecmwf/.test(O.product) && A(O.overlay) && s.unshift(O.product);
		...
		function D() {
			if (!b) {
				var e = t.get("overlay"),
					n = l[e].getName();
				"wind" === e ? (L = null, document.title = v) : (L = n, document.title = "Windy: " + n)
			}
		}
		return s.on("paramsChanged", function (e) {
			O = e, R()
		}), s.on("mapChanged", R), s.on("detailRendered", R), o.on("pickerOpened", R), o.on("pickerMoved", R), o.on("pickerClosed", R), t.on("particlesAnim", R), t.on("overlay", D), t.on("usedLang", D), {
			...
			getURL: function () {
				return "https://www.windy.com/" + C()
			},
			deleteSearch: function () {
				return u = null
			},
			url: function (e, t) {
				void 0 === t && (t = null), p = e, S = t, x()
			},
			title: function (e) {
				L = e, document.title = "Windy: " + e, b = !0
			},
			reset: function () {
				b = !1, y && (y.content = w), p = "", S = null, D(), x()
			}
		}
	}), /*! */
			...
			if (37 !== c && 39 !== c || !(n = r()))
				if (38 === c || 40 === c) {
					for (var u = s.get("favOverlays"), h = s.get("overlay"), m = a.overlays.indexOf(h), f = a.overlays.length; m >= 0 && m < f;)
						if (m += 38 === c ? -1 : 1, u.includes(a.overlays[m])) return void s.set("overlay", a.overlays[m]);
					d = !0
		...
		n.isMobile && r.instance({
			...
			render: function () {
				...
				if (this.calendar = s.get("calendar"), this.zuluMode = s.get("zuluMode"), this.localeHours = a.getHoursFunction(), this.timestamp = 0, this.calendar) {
					...
					delete this.box.dataset.zulu, this.set(f), this.renderBox(), e("b", this.scrollEl).style.left = window.innerWidth / 2 + (Date.now() - this.calendar.start) / this.tsPx + "px"
				}
			},
			slideUp: function () {
	...
	W.define("promo", ["utils", "$", "rootScope", "broadcast", "storage", "plugins", "geolocation", "log"], function (e, t, n, i, s, a, r, o) {
		...
		if (i.once("dependenciesResolved", function () {
			...
			var m = document.body.classList;
			n.sessionCounter < 20 || n.sessionCounter >= 20 && !o.has("pl/menu") ? i.once("hpHidden", function () {
				n.user || (m.add("mm-show"), m.remove("mm-hide"), setTimeout(function () {
					return m.add("mm-smaller")
				}, 3e3), setTimeout(function () {
					return m.remove("mm-show")
				}, 1e4), setTimeout(function () {
					return m.add("mm-hide")
				}, 13e3))
			}) : o.has("ov/satellite") || o.has("ov/radar") || i.once("hpHidden", function () {
				m.add("mm-radar-promo"), m.add("mm-smaller"), setTimeout(function () {
					return m.remove("mm-smaller")
				}, 2e3), setTimeout(function () {
					return m.add("mm-smaller")
				}, 6e3), setTimeout(function () {
					return m.remove("mm-radar-promo")
				}, 8e3)
			})
		}
		return {
	...
	W.define("user", ["rootScope", "broadcast", "http", "storage"], function (e, t, n, i) {
		...
		return n.get("https://node.windy.com/users/info", {
			cache: !1,
			withCredentials: !0
		}).then(s).catch(r), {
			getAvatar: a,
	...
	W.define("timeAnimation", ["utils", "store", "products", "broadcast"], function (e, t, n, i) {
		...
		t.on("animation", function (e) {
			e !== r && (e ? function () {
				...
				r = !0, a = s = t.get("path"), c = 2, t.on("visibility", h), t.on("product", p), t.on("overlay", f), i.on("redrawFinished", m), i.on("paramsChanged", g), i.on("pluginOpened", p), v(), i.emit("animationStarted")
			}() : (r = !1, clearTimeout(o), t.off("visibility", h), t.off("product", p), t.off("overlay", f), i.off("redrawFinished", m), i.off("paramsChanged", g), i.off("pluginOpened", p)))
		});
		var h = function (e) {
				e || p()
			},
	...
	W.define("hp", ["map", "utils", "store", "query", "rootScope", "geolocation", "http", "broadcast", "storage"], function (e, t, n, i, s, a, r, o, l) {
		...
		return s.isResumed || !u || s.startupDetail ? document.body.classList.remove("onweather") : m(), o.on("back2home", function () {
			...
			return function (e, t, n, i) {
				...
				switch (h) {
					case 0:
						...
						break;
					case 1:
						u(m, w, b, T), u(m + 4, g, v, y), u(m + 1024, g, v, y), u(m + 1028, g, v, y);
						break;
					case 2:
						u(m, w, b, T), u(m + 4, w, b, T), u(m + 1024, g, v, y), u(m + 1028, g, v, y);
						break;
					case 3:
						u(m, w, b, T), u(m + 4, w, b, T), u(m + 1024, w, b, T), u(m + 1028, g, v, y);
						break;
					case 4:
						d(e, t, i)
				}
	...
	W.define("renderers", ["Renderer", "testWebGl", "map", "plugins", "tileLayer", "utils", "tileInterpolator"], function (e, t, n, i, s, a, r) {
		...
		l.tileLayer = o.instance(), l.tileLayerPatternator = o.instance({
			...
			onopen: function () {
				this.onopen = i.satellite.onopen, this.onclose = i.satellite.onclose, this.redraw = i.satellite.onredraw, this.interpolator = i.satellite.interpolator
			}
				...
				var a = this,
					r = function (e, i) {
						...
						var y = g[v],
							...
							N = f * f,
							k = (y * _ + w * R + E * O + A * D) / N,
							I = s.interpolateNearestG ? t.interpolateNearest(null, 0, b, T, C, M, _, R, O, D) : (b * _ + T * R + C * O + M * D) / N,
							W = (L * _ + S * R + x * O + P * D) / N;
	...
	W.define("renderTile", ["render", "layers"], function (e, t) {
		return function (n, i, s, a, r) {
			if (s === this.syncCounter) {
				...
				var m, f, p, g, v, y, w, b, T, L, S, E, A = !1,
					C = 0 | a.trans,
					M = 0 | Math.log2(C),
					x = 0 | Math.log2(C * C),
					P = 0 | a.intX,
					...
					te = r.decodeR,
					ne = r.decodeG;
				for (l ? (T = e.createCombinedFillFun(u, J.getColor(), J.getColor2(), J.getAmountByColor), L = e.createFillFun(u, n, J.getColor())) : T = L = e.createFillFun(u, n, J.getColor()), K && (te = r.decodeB), E = 0; E < 256; E += n)
					for (B = E - ((F = E >> M) << M), S = 0; S < 256; S += n) H = S - ((U = S >> M) << M), G !== U && (N = 2056 + U + k + (((z = F + I) << 8) + z) << 2, void 0 !== h && (A = h(c, N)), !0 === K && (N += 2), j = c[N], V = c[N + 4], Y = c[N + 1028], q = c[N + 1032], !0 === ee && (Z = c[N + 1], X = c[N + 5], $ = c[N + 1029], Q = c[N + 1033]), G = U), A ? L(S, E, NaN) : (w = te(null !== O ? j * O[D = H + (B << M) << 2] + V * O[D + 1] + Y * O[D + 2] + q * O[D + 3] >> x : j * (m = (1 - (v = H / C)) * (1 - (y = B / C))) + V * (f = v * (1 - y)) + Y * (p = y * (1 - v)) + q * (g = v * y)), !0 === ee && (b = ne(null !== O ? Z * O[D] + X * O[D + 1] + $ * O[D + 2] + Q * O[D + 3] >> x : Z * m + X * f + $ * p + Q * g)), l ? T(S, E, w, b) : T(S, E, ee ? Math.sqrt(w * w + b * b) : w));
				d.putImageData(e.imgData, 0, 0), "pattern" in o && o.pattern in W && W[o.pattern].addPattern(d, C, c, 2056, k, I, R, te, ne)
			}
		}
	}), /*! */
	...
	W.define("Favs", ["storage", "utils", "Class"], function (e, t, n) {
		return n.extend({
			...
			add: function (e) {
				...
				return "alert" === e.type ? i.alertId = e.alertId : "airport" === e.type ? i.icao = e.icao : "station" === e.type ? i.stationId = e.stationId || e.id || e.key : "webcam" === e.type && (i.webcamId = e.webcamId), this.data[n] = i, this.onchange(), this.save(), !0
			},
			isFav: function (e) {
		...
		return L.GridLayer.extend({
			...
			renderTile: function (e, t, n) {
				for (var s = this._getTilePos(t), a = s.x, r = s.y, o = this._map.getPixelOrigin(), l = o.x, c = o.y, d = 256 << t.z, u = [], h = 0; h < n.length; ++h) {
					...
					A.textContent = A.dataset.label = p, A.dataset.id = L, A.dataset.poi = "label", A.className = g, A.style.transform = "translate(" + S + "px, " + E + "px)", A.style.width = w + "px", T || u.push({
						id: L,
						el: A
					}), e.appendChild(A)
				}
			...
			renderWeather: function (e, t, n) {
				...
				if (s)
					if (a && a.length) {
						...
						o >= 0 && o < a.length ? s.dataset.temp = this.tempConverter(a[o]) + "Â°" : delete s.dataset.temp
	...
	W.define("map", ["render", "plugins", "WindyMap", "rootScope", "utils", "store", "broadcast", "geolocation", "router"], function (e, t, n, i, s, a, r, o, l) {
		...
		}), g(), v(), p(a.get("graticule")), c.on("moveend", g), c.on("movestart", r.emit.bind(r, "movestart")), r.on("zoomIn", c.zoomIn, c), r.on("zoomOut", c.zoomOut, c), r.on("updateBasemap", v), a.on("map", v), a.on("graticule", p), e.on("toggleSeaMask", function (e) {
		...
		function v() {
			...
			}, c.baseLayer && c.removeLayer(c.baseLayer), c.baseLayer = L.tileLayerMulti(n, {
				detectRetina: !1,
				minZoom: 3,
				maxZoom: 18,
				updateWhenIdle: !!i.isMobileOrTablet,
	...
	W.define("pois", ["Poi", "utils", "broadcast", "favs", "singleclick"], function (e, t, n, i, s) {
		...
		return a.empty = e.instance({
			...
			onclick: function (e, s) {
				...
				o.source = "poi-icon", n.emit("rqstOpen", l, o)
			},
			cancel: function () {}
		}), a
	}), /*! */
	W.define("poisCtrl", ["pois", "store", "utils", "broadcast", "plugins"], function (e, t, n, i, s) {
		...
		function r(e) {
			"favs" !== e && "empty" !== e ? (t.off("pois", r), s["poi-libs"].open().then(function () {
				o(e);
